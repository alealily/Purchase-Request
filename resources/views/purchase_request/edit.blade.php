...
<div class="flex bg-[#F2F1F1] min-h-screen">
    ...
    <div class="flex-1 p-10">
        ...
        <div class="bg-[#187FC4] text-white rounded-2xl mb-8 py-4 px-6">
            <h1 class="font-bold text-xl">Edit Purchase Request #{{ $pr->pr_number }}</h1>
        </div>

        {{-- Form Card --}}
        <div class="bg-white p-8 rounded-2xl shadow-sm">
            <form action="{{ route('purchase_request.update', $pr->id_pr) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" 
                               name="material_desc" 
                               value="{{ old('material_desc', $pr->prDetails->material_desc ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               value="{{ $pr->prDetails->currency_code ?? 'RP' }}" 
                               disabled
                               class="w-full border border-gray-300 px-4 py-2 rounded-lg bg-gray-100 text-gray-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="unit_price" 
                               value="{{ old('unit_price', $pr->prDetails->unit_price ?? '') }}"
                               class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                               placeholder="0"
                               min="0"
                               required>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="quantity" 
                               value="{{ old('quantity', $pr->prDetails->quantity ?? '') }}"
                               class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                               placeholder="0"
                               min="1"
                               required>
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               value="{{ number_format($pr->prDetails->total_cost ?? 0, 0, ',', '.') }}">
                    </div>
                        ...
                        <select name="id_supplier" 
                            ...
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id_supplier }}" 
                                    {{ old('id_supplier', $pr->prDetails->id_supplier ?? '') == $supplier->id_supplier ? 'selected' : '' }}>
                                    {{ $supplier->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @if($pr->prDetails && $pr->prDetails->quotation_file)
                        @php
                            $existingFiles = json_decode($pr->prDetails->quotation_file, true);
                            if (!is_array($existingFiles)) {
                                $existingFiles = [$pr->prDetails->quotation_file];
                            }
                        @endphp
                        <div class="col-span-2">
                            <label class="block text-sm font-semibold mb-2">Current Quotation Files</label>
                                        ...
                                        <li class="flex items-center gap-2 mb-1">
                                            <a href="{{ url('/storage/quotations/' . $file) }}" 
                                               ...
                                               class="text-blue-600 hover:underline">
                                                <i class="fa-solid fa-file-pdf mr-1"></i>{{ $displayName }}
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">Upload new files below to replace current files (optional)</p>
                        </div>
                    @endif

                    {{-- Quotation Files (Max 3) --}}
                    <div class="col-span-2">
                        <label class="block text-sm font-semibold mb-2">Upload New Quotation Files (PDF, DOCX) - Max 3 files</label>
                        <input type="file" 
                        ...
                        <p class="text-xs text-gray-500 mt-1">Leave empty to keep current files. You can select up to 3 files (PDF or Word). Max 10MB each.</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateFileList() {
            ...
            if (files.length > 0) {
                let html = '<strong>New files to upload (' + files.length + '/' + MAX_FILES + '):</strong><ul class="list-disc ml-4 mt-1">';
